<div class="container d-flex align-items-center justify-content-center" style="min-height: 524px">
    <div class="text-center">
        <h1 class="display-4 mb-4">Welcome to McShop</h1>
        <p class="lead mb-3">Lorem ipsum dolor, sit amet consectetur adipisicing elit. Rem praesentium doloremque natus
            excepturi quae voluptatum distinctio nisi, voluptatibus accusamus ex nostrum similique quisquam, vel modi
            repellendus perferendis asperiores corporis mollitia!</p>
        <p class="mb-4">Check out our products</p>
        <a href="{{ url('/products') }}" class="btn btn-primary">Products</a>
    </div>
</div>
